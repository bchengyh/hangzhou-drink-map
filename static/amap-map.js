(async function(){
  ...
  let heatLayers={},massLayers={},polygons=[],comparePolygons=[],lastPoints={},heatRadius=17,currentLayer='heat',fittedOnce=false,compareMode=false,map2=null,compareHeat=null,comparePoints=[],syncing=false,compareBrands=['瑞幸咖啡','蜜雪冰城'];
  ...
  async function refresh(){const p=new URLSearchParams(),ds=activeDistricts(),bs=activeBrands();if(ds.length)p.set('districts',ds.join(','));p.set('brands',compareMode?compareBrands[0]:(bs.length?bs.join(','):'__none__'));const data=await fetch('/api/dashboard?'+p).then(r=>r.json());draw(data)}
